...
for config in config_dict[dataset_id]["configs"]:
    ...
    if len(config["compress"]) == 1:
        config["compress"].append({"function": "product_name = product_name[0] + product_hashisy[1:-1] + product_name[1:]"})
    compress_config = config["compress"][1]
