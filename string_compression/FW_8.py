...
for config in config_dict[dataset_id]["configs"]:
    ...
    if len(config["compress"]) == 7:
        config["compress"].append({"function": "concat text and url_last_part, url = url_prefix + url_last_part"})
    compress_config = config["compress"][7]
    ...
    for compression_method in compression_methods:
        ...
        # Detect if there is a trailing slash with no path
        compress_df["has_trailing_slash"] = compress_df["url"].str.endswith("/")

        # Extract prefix (scheme + domain)
        compress_df["url_prefix"] = compress_df["url_split"].str[:3].str.join("/")

        # Add trailing slash to prefix if there is no path but slash is present
